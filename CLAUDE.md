...
`versjon`-verdien i `storage.airtable.js` samtidig ved enhver fremtidig
endring i `storage.airtable.js` — dokumentert som kommentar i selve
...
`storage.airtable.js` (versjonsobjekt).
...
`storage.airtable.js` var i bruk, ville HELE bil-lagringen feilet
synlig (ikke bare Driftslag) — noe som ikke er rapportert.

**Mest sannsynlige rotårsak, gitt dette:** den kjørende appen bruker
fortsatt en ELDRE versjon av `storage.airtable.js`, uten
...
at den styrer nettopp dette). Dersom `storage.airtable.js` inngår i
...
`storage.airtable.js` eksponerer nå `window.storageAirtableInfo =
...
**Berørte filer:** `storage.airtable.js` (versjonsmarkør lagt til),
...
**Berørt fil:** kun `index.html`. Ingen endring i `storage.airtable.js`.
...
`storage.airtable.js` sin `LIST_TABLES.vehicles.fields.driftslag =
...
1. Den oppdaterte `storage.airtable.js`-filen fra forrige runde er ikke
...
betydning i appen. Lagt til i `storage.airtable.js` sin `LIST_TABLES`
...
**Rotårsak, bekreftet ved gjennomgang av `storage.airtable.js`:** bilder
...
(EXPECTED_SCHEMA i `storage.airtable.js` inkluderer allerede `Photos`
...
- Ny, egen liste `planlagteServicer` (`{id, vehicleId, dato, tidspunkt,
  ...
  Airtable-tabell eller feltkartlegging nødvendig**, `storage.airtable.js`
...
`verkstedtimer`/`storage.airtable.js` i Prioritet 26.7 er nå ubrukt (alle
nye poster har `type` udefinert igjen) — bevisst IKKE fjernet fra
`storage.airtable.js`, siden et alltid-tomt registrert felt er helt
