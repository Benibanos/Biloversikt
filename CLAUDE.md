...
**Dashboard V4 W1 (2026-09-26):** standard layoutrekkefølge og lås på Operativ etterlevelse og Verkstedoppfølging. Mobil uten ekstra banner. Appversjon 136. Motorene urørt. `storage.airtable.js` uendret v2.26.1. Før det: **Verkstedoppfølging v1 (2026-09-26):** `WorkshopFollowupEngine` er eneste avledede oppfølgingsmotor (ikke verkstedsvalg, ikke automatisk booking/lukking/tilbake i drift). `vtStatus()` er eneste verkstedstatus. Oppfølgingsstater i ordbok §12, aldri lagret. `AlertEngine` er fortsatt eneste varselmotor. Appversjon 135. `storage.airtable.js` uendret v2.26.1. Før det: Flåtehelse v1 (2026-09-26): `flatehelse()` er eneste scoring (`../Dv4/project/fleet-health-framework.md`). Måler teknisk tilstand og tilgjengelighet. Kontrollgrad, EU-frist og serviceintervall er operativ etterlevelse (`operativEtterlevelse()`), ikke flåtehelse. Varsellampe-matrise i `ALVORLIGHET_VARSELLAMPE`. Fase 1B-felt er `aktiv:true`. Tom single select utelates fra PATCH. Bare tomme felt skrives ved 1B-migrering. Ingen automatisk 1B-kjøring. Datakvalitetsmotor er ikke startet. Før det: Implementeringsfase 1B migrering (2026-09-25). Før det: Implementeringsfase 1E (2026-09-26): `vtStatus()` er eneste verkstedstatus. Statusordboken er eneste kilde til statusnavn, farger, ikoner og alvorlighet.
...
1. **Én statusordbok.** `STATUS` / `STATUS_TONE` / `STATUS_ALIAS` (rett før «Aktive Saker: konstanter») er den ENESTE definisjonen av statusnavn, toner og ikoner for domenene i `status-dictionary.md` (driftsstatus, operativ, verksted, alvorlighet, synk, utboks, sak, skade, varsellampe, EU, varsel, skadeBilde, kmGodkjenning, loftebordType, flatehelse, datakvalitet). Bruk `statusLabel()`, `statusTone()`, `statusPilleHtml()`, `statusIkonHtml()`, `statusEmoji()`, `statusStil()`, `statusOptionsHtml()`. Ingen ny kode definerer egne statusord, farger eller ikoner; en ny status legges først inn i designsystemets ordbok, deretter i `STATUS`.
2. **Gamle nøkler er alias.** `vehicleHovedstatus()` returnerer ordbokens nøkler (`kan-ikke-brukes`, `verksted-planlagt` …). `ute-av-drift` og `verksted` virker fortsatt via `STATUS_ALIAS` (lagrede filtre, eldre kode). Det finnes ingen parallelle label-/ikontkart: visning går gjennom `statusLabel()`/`statusEmoji()`/`statusPilleHtml()`/`statusStil()`. `HOVEDSTATUS_ORDER` og `SAK_STATUS_ORDER` er kun rekkefølge, ikke tekst.
...
8. **Varselmotor.** `AlertEngine` er eneste byggesteg (`alle` / `senter` / `dashboard` / `forKjoretoy`). Hvert varsel har `alvorlighet` og `varselStatus`. Klokke, sideteller og Dashboard = kun Kritisk og Høy (`AlertEngine.dashboard()`). Km-avvik er datakvalitet. Informasjonshendelser er aldri varsler: sjåførkommentarer og sjåførnotater vises i «Siste aktivitet» (`aktivitetsHendelser()`, `dashboard.aktivitet`) og i Kommentaroversikt. Wrappere `beregnVarslingssenterListe()` / `varslingssenterKlokkeListe()` peker på motoren. Ny varseltype i matrisen først, deretter i `AlertEngine.alle()`.
