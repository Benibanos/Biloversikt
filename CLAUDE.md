...
**Flåtehelse v1 (2026-09-26):** `flatehelse()` er eneste scoring (`../Dv4/project/fleet-health-framework.md`). Måler teknisk tilstand og tilgjengelighet. Kontrollgrad, EU-frist og serviceintervall er operativ etterlevelse (`operativEtterlevelse()`), ikke flåtehelse. Varsellampe-matrise i `ALVORLIGHET_VARSELLAMPE`. Appversjon 134. Fase 1B-felt er `aktiv:true` (`storage.airtable.js` v2.26.1). Tom single select utelates fra PATCH. Bare tomme felt skrives ved 1B-migrering. Ingen automatisk 1B-kjøring. Datakvalitetsmotor er ikke startet. Før det: Implementeringsfase 1B migrering (2026-09-25). Før det: Implementeringsfase 1E (2026-09-26): `vtStatus()` er eneste verkstedstatus. `AlertEngine` er appens eneste varselmotor. Statusordboken er eneste kilde til statusnavn, farger, ikoner og alvorlighet.
