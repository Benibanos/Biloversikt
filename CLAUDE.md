...
**Implementeringsfase 1B migrering (2026-09-25): manuell «Kjør migrering» i Optimaliseringer → Airtable.** Fase 1B-felt er `aktiv:true` (`storage.airtable.js` v2.26.0). Bare tomme felt skrives; eldre felt og `STATUS_ALIAS` beholdes. Dobbeltskriving Driftsstatus/UteAvDrift og Status/Utfort. 14 dagers kompatibilitet etter vellykket kjøring. Ingen automatisk kjøring. Fase 2, flåtehelse og datakvalitetsmotor er ikke startet. Appversjon 132. Før det: tørrkjøring (app 131). Før det: Implementeringsfase 1E (2026-09-26): `vtStatus()` er eneste verkstedstatus (`workshop-state-model.md`). `AlertEngine` er appens eneste varselmotor. Dashboard, Varslingssenter og Kjøretøyprofil leser AlertEngine (matrise: `../Dv4/project/alert-classification-matrix.md`). Statusordboken er eneste kilde til statusnavn, farger, ikoner og alvorlighet.
