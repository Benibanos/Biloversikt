...
**Implementeringsfase 1B tørrkjøring (2026-09-25): skrivefri migreringsforhåndsvisning i Optimaliseringer → Airtable (`fase1BMigreringForhandsvis()`).** Ingen Airtable-skriving, ingen migrering, ingen dobbeltskriving. Fase 1B-felt forblir `aktiv:false`. Appversjon 131; `storage.airtable.js` uendret v2.25.0. Før det: Implementeringsfase 1E (2026-09-26): `vtStatus()` er eneste verkstedstatus (`workshop-state-model.md`). `AlertEngine` er appens eneste varselmotor. Dashboard, Varslingssenter og Kjøretøyprofil leser AlertEngine (matrise: `../Dv4/project/alert-classification-matrix.md`). Statusordboken er eneste kilde til statusnavn, farger, ikoner og alvorlighet.
