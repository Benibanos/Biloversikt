...
Sist oppdatert: 2026-09-26 (Implementeringsfase 1A godkjent — statusordbok, alvorlighetsregel og varselmotor i appen, sjåførkommentarer flyttet til «Siste aktivitet», uten Airtable-endring; appversjon 126; venter på manuell applikasjonstest, se seksjonen under).

## Implementeringsfase 1 — status (2026-09-26)

**Fase 1A — godkjent i kode, venter på manuell applikasjonstest (appversjon 126).** Statusordbok (`STATUS`), kjøretøystatus med «Kan ikke brukes», verkstedstatus (`vtStatus()`), alvorlighetsregel for skader med «Kan bilen kjøres?», varselmotor med alvorlighet og varselstatus, synkstatus, sjåførkommentarer i «Siste aktivitet» i stedet for Varslingssenteret. Detaljer: `CHANGELOG.md` 2026-09-26.

**Fase 1B starter ikke før alle scenariene under er bestått.** Test etter publisering, helst med en testbil (f.eks. en reservebil) slik at ekte drift ikke påvirkes. Rydd opp etterpå: sett testbilen tilbake i drift og lukk testsakene. Kryss av og noter dato, tester og avvik.
...
**Trinn B — venter på kolonner i Airtable.** Kolonneliste og rekkefølge: `AIRTABLE_MIGRATION.md`, «Implementeringsfase 1, Trinn B». Etter at kolonnene finnes: lagret `Driftsstatus` (inkl. Utfaset), `WorkshopAppointments.Status` (Bekreftet, Pågår, Avlyst), `Damages.Skadetype`/`KanKjores`, `AktiveSaker.Alvorlighet`, `Users.Tilgangsrolle`, migreringsfunksjon for administrator og persistert synkstatus.
